...
startup
{
	try{
		...
		vars.pausedIGTime = TimeSpan.Zero;
		vars.pauseStartTime = DateTime.Now;
		vars.gameTime = TimeSpan.Zero;
		System.Net.Sockets.TcpListener listener = new System.Net.Sockets.TcpListener(12347);
						...
						switch(cmd[0] - '0')
						{
							...
							case 4:
								if (!vars.loading) {
									...
									vars.pausedIGTime = vars.gameTime;
									vars.pauseStartTime = DateTime.Now;
								}
								break;
							case 5:
...
{
	if (vars.loading) {
		return vars.pausedIGTime == TimeSpan.Zero ? vars.pausedTime : vars.pausedIGTime;
	}
